...
def cal_tp_fp_distribution(client: Client=None):
    print('start')
    step = 10
    FP_Prefix = 'FP'
    TP_Prefix = 'TP'
    query = """
            cluster('https://slivdevoutput00usw3.westus3.kusto.windows.net/').database('outputs').SLISustainedDuration_v2
            ...
            |summarize count()
          """
    df = pd.DataFrame(columns=['low_sparse_rate', 'high_sparse_rate', 'fp_count', 'tp_count','fp_query','tp_query'])
    i = 0
    for low_sparse_rate in tqdm(range(0, 100, step)):
        ...
        df.loc[i,'fp_query'] = fp_query
        df.loc[i,'tp_query'] = tp_query
        i = i + 1
